...
main:
    ...
    mul $t2, $t0, 24        # multiply x by the number of columns   
    mul $t3, $t2, 4         # multiply by size of word
    la $t4, array           # load start of array
    add $t5, $t4, $t3       # move to the correct row
    
    mul $t2, $t1, 4         # multiply y by size of word 
    add $t6, $t5, $t2       # move to the correct column
    
    lw $a0, ($t6)           # printf("%d", numbers[i])
    li $v0, 1               
    syscall
