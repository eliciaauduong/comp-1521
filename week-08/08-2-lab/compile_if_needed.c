...
int is_compile_needed(char *c_file, char *binary) {
    ...
    struct stat c; // load stats for c_file
    stat(c_file, &c);
    
    struct stat b; // load stats for binary
    if (stat(binary, &b) != 0) { // return 1 if binary does not exist
        return 1;
    }
    
    // compare modification times for c_file and binary
    if (c.st_mtime > b.st_mtime) { // if c_file is more recent return 1
        return 1;
    } else {
        return 0;
    }
